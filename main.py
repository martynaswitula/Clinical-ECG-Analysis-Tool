...
def show_data(file_path):
    global canvas,selected_file,nazwa # Deklarujesz `canvas` jako zmienną globalną, aby mieć do niej dostęp w funkcji
    try:
        # Jeśli `canvas` istnieje, usuń go
        if canvas:
        ...
        canvas.draw()
        # Tkinter może mieć problemy z wielokrotnym wyświetlaniem wykresu, więc wywołaj canvas.get_tk_widget().pack() ponownie
...
def isChecked():
    ...
    if Checkbutton1.get()==1:
        ...
        # Assuming the signal is sampled at a fixed rate, e.g., 1000 samples per second
        fs = 160
        # Find peaks in the ECG signal based on the voltage column
        peaks, _ = find_peaks(df['napięcie'], height=0.5, distance=fs)

        # Refine peaks using peak prominence
...
def compute_measures(r_peaks): # przyjmuje jako argumnety załamki R (tablica zawierjaca załąmki)
    measures = {}
    #measures['Average_RR'] = np.mean(np.diff(r_peaks)) # ŚREDNI ODSTĘP MIĘZY ZAŁAMKAMI R
...
def calculate_heart_rhythm(file_path):
    try:
        ...
        # Znajdź załamki R na podstawie sygnału EKG
        peaks, _ = find_peaks(df['napięcie'], height=0.5, distance=160)  # fs=160 Hz

        # Sprawdź, czy rytm serca jest regularny czy nieregularny
...
# Nowa funkcja do zapisu wyników do pliku po kliknięciu przycisku "Zapisz"
def zapisz_wynik():
    global selected_file
    try:
        result = None
...
wybierz_label = Label(page2_administrator, text='Wybierz i wczytaj plik EKG:', font=("Arial", 14, "bold"))
wybierz_label.grid(row=0, columnspan=2, pady=(50, 20))
...
button_zapisz = Button(page5_administrator, text="Zapisz", command=zapisz_wynik, width=12, bg='green', fg='white', font=("Arial", 10))
button_zapisz.grid(row=4, column=0, pady=10, padx=10)
...
wybierz_label = Label(page3_sygnal, text='Wybierz i wczytaj plik EKG:', font=("Arial", 14, "bold"))
wybierz_label.grid(row=0, columnspan=2, pady=(50, 20))

wybierz_plik = Button(page3_sygnal, text="Kliknij i wybierz plik txt", command=load_file, font=("Arial", 12))
wybierz_plik.grid(row=1, column=0, columnspan=2, sticky='NSEW', padx=10, pady=10)
...
wybierz_label = Label(page6_sygnal, text='Wybrany plik EKG:', font=("Arial", 14, "bold"))
wybierz_label.grid(row=0, column=0, columnspan=2, pady=(50, 20))

nazwa_pliku_label = Label(page6_sygnal, textvariable=nazwa, font=("Arial", 12))
nazwa_pliku_label.grid(row=1, column=0, columnspan=2)

sygnal_label = Label(page6_sygnal)
sygnal_label.grid(row=2, column=0, columnspan=2)
...
analiza_button = Button(page6_sygnal, text='Analizuj', command=wyswietl_sygnal, width=12, bg='navy', fg='white', font=("Arial", 10)) 
analiza_button.grid(row=4, column=1, pady=10, padx=(0, 10), sticky='e')

def goBackToPage2():
    page2_administrator.tkraise()

back_button = Button(page6_sygnal, text='Wróć', command=goBackToPage2, bg='navy', fg='white', font=("Arial", 10), width=10)
back_button.grid(row=4, column=0, pady=10, padx=(10, 0), sticky='w')
...
#### DRUGIE OKNO ANALIZY
page4_wynik_analizy.rowconfigure(0,weight=1)
page4_wynik_analizy.rowconfigure(1,weight=1)
page4_wynik_analizy.rowconfigure(2,weight=1)
page4_wynik_analizy.rowconfigure(3,weight=1)
page4_wynik_analizy.rowconfigure(4,weight=1)
page4_wynik_analizy.rowconfigure(5,weight=1)
page4_wynik_analizy.rowconfigure(6,weight=1)
page4_wynik_analizy.rowconfigure(7,weight=1)
page4_wynik_analizy.rowconfigure(9,weight=1)

page4_wynik_analizy.columnconfigure(0,weight=1)
page4_wynik_analizy.columnconfigure(1,weight=1)
page4_wynik_analizy.columnconfigure(2,weight=1)

plik_podany_analize=Label(page4_wynik_analizy,text="Plik EKG poddawany analizie:")
plik_podany_analize.grid(row=0, column=1, sticky='NW')

nazwa_pliku_label = Label(page4_wynik_analizy, textvariable=nazwa, font=("Arial", 12, "bold"))
nazwa_pliku_label.grid(row=1, column=1, sticky='NW')

wybrana_metoda=Label(page4_wynik_analizy, text='Wybrana analiza:')
wybrana_metoda.grid(row =2, column=1,sticky='NW')

wynik_analizy=Label(page4_wynik_analizy, text='Wynik analizy:')
wynik_analizy.grid(row =3, column=0,sticky='NW')

def goBackToPage4():
    page3_wyswietla_syganl.tkraise()

back_button = Button(page4_wynik_analizy, text='Wróć', command=goBackToPage4, bg='navy', fg='white', font=("Arial", 10), width=10)
back_button.grid(row=11, column=0, pady=10, padx=(10, 0), sticky='w')
